...
### 经典node的行为  
- 可被以任一node作参数索引，返回一个值，相同node的索引结果必定为同一个值，且值未被更改时内容为`nil`  
- 可被求值，不做任何事并返回一个经典值，内容为自身  
- 可被调用，不做任何事并返回一个经典值，内容为自身  
- 可被以任一node作参数替换，所有指向此node的引用都被**强制**更改为replace指定的引用，如同这个node本身就是替换指定的值一般  
...
### s表达式  
对于s表达式`(- 1000 7)`，如同lisp语言一般看做列表长度3、元素依次为`-`、`1000`、`7`的列表  
* 以node表示为`(- 1000 7)`，暨：  
  - car为`-`，cdr为`(1000 7)`，暨：  
    * car为`1000`，cdr为`(7)`，暨：  
      - car为`7`，cdr为特殊值`nil`。  
...
- 要求：可被求值，返回值规则如下：
  * 若cdr不为`nil`，求值car并丢弃结果，返回其cdr的求值结果  
  * 若cdr为`nil`，求值car并返回结果  
### 函数  
